...
function loadStoredOrders() {
  try {
    const orders = JSON.parse(localStorage.getItem("sonyShopOrders") || "[]");
...
function persistOrders() {
  try {
    localStorage.setItem("sonyShopOrders", JSON.stringify(shopState.orders));
  } catch (error) {}
}
...
function getCheckoutSummary() {
  ...
  const productTotal = getCheckoutProductTotal();
  ...
  return {
    ...
    items: shopState.cart.map((product) => ({
      title: product.title,
...
function openOrders() {
  renderOrders();
  closeCart();
  shopRefs.orders.classList.add("is-open");
  shopRefs.orders.setAttribute("aria-hidden", "false");
}
...
function renderOrders() {
  ...
  shopRefs.ordersList.innerHTML = shopState.orders.map((order) => {
    ...
    return `
      <article class="shop-order-card${shopState.orderManageMode ? " is-manageable" : ""}${isSelected ? " is-selected" : ""}" data-order-id="${escapeHTML(order.orderId)}">
...
function createDetailSection(section) {
  ...
  const kicker = createNode("p", "product-detail-section-kicker", section.kicker);
  ...
  block.append(kicker, title, body, list);
  return block;
}
...
function openProductDetail(model, trigger) {
  ...
  const eyebrow = createNode("p", "product-detail-eyebrow", detail.eyebrow);
  ...
  content.append(eyebrow, title, tagline, summary, stats, sections);
...
function updateCheckoutTimer() {
  ...
  checkoutRemainingSeconds -= 1;
}

function startCheckoutTimer() {
  stopCheckoutTimer();
  clearPaymentSimulationTimer();
  checkoutRemainingSeconds = checkoutDurationSeconds;
...
function showPaymentSuccess() {
  ...
  shopState.orders.unshift(summary);
  persistOrders();
  stopCheckoutTimer();
  shopState.cart = [];
  shopState.pendingRemovalSku = null;
...
syncProductFromQuery();
bindCheckoutAddressFields();
